...
def sa_tiers():
	for t in Ticker.objects.all():
		sa_tier_ones = ['DDD', 'ATVI', 'AMZN', 'AAPL', 'ATW', 'BRK-B',
		'SAM', 'CNR', 'KMX', 'CMG', 'CGNX', 'CLB', 'GLW', 'COST', 'CMI', 'DASTY', 'DISCK',
		'EBAY', 'ESRX', 'FB', 'FEYE', 'F', 'GILD', 'GOOGL', 'HAS', 'IBKR', 'KMI', 'LUK',
		'LNKD', 'LGF', 'MA', 'NOV', 'NFLX', 'NKE', 'NVDA', 'PNRA', 'PYPL', 'SNI', 'SWIR', 'SBUX', 'TSLA',
		'TCS', 'HAIN', 'PCLN', 'DIS', 'TDG', 'TRIP', 'UA', 'UNH', 'VRX', 'WPRT', 'WFM']
		if t.ticker_symbol in sa_tier_ones:
			one = 1
			t.tier = one
			print t.ticker_symbol, t.tier
		else:
...
def coverage():
	...
	'Fool.com Earnings Preview', 'Team Earnings (RB)', 'Potential 2-Minute Drill (RB)']	
	#RYR = nothing
		...
		if t.services_for_ticker:
			...
			for service_pretty_name in service_pretty_names:
				...
				if service_pretty_name in TIER_1_COVERAGE_KEYED_BY_SERVICE_PRETTY_NAME:
						promised_coverage_for_this_ticker += TIER_1_COVERAGE_KEYED_BY_SERVICE_PRETTY_NAME
						print promised_coverage_for_this_ticker
